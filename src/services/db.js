  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(DB_STORES.categories)) {
        const categoryStore = db.createObjectStore(DB_STORES.categories, {
          keyPath: "id",
          autoIncrement: true,
        });
      ...
      if (!db.objectStoreNames.contains(DB_STORES.dishes)) {
        const dishStore = db.createObjectStore(DB_STORES.dishes, {
          keyPath: "id",
          autoIncrement: true,
        });
...
export const db = {
  ...
  categories: {
    getAll: () =>
      dbOperation(DB_STORES.categories, "readonly", (store) => store.getAll()),
    add: (category) =>
      dbOperation(DB_STORES.categories, "readwrite", (store) =>
        store.add(category)
      ),
    update: (category) =>
      dbOperation(DB_STORES.categories, "readwrite", (store) =>
  ...
  dishes: {
    getAll: () =>
      dbOperation(DB_STORES.dishes, "readonly", (store) => store.getAll()),
    getByCategory: async (categoryId) => {
      const all = await dbOperation(DB_STORES.dishes, "readonly", (store) =>
        store.getAll()
      );
      return categoryId !== null
        ? all.filter((dish) => dish.categoryId === categoryId)
        : all;
    },
    add: (dish) =>
      dbOperation(DB_STORES.dishes, "readwrite", (store) =>
        store.add({
          ...dish,
          comments: [],
        })
    ...
    update: (dish) =>
      dbOperation(DB_STORES.dishes, "readwrite", (store) => store.put(dish)),
    delete: (id) =>
      dbOperation(DB_STORES.dishes, "readwrite", (store) => store.delete(id)),
    getById: (id) =>
      dbOperation(DB_STORES.dishes, "readonly", (store) => store.get(id)),
    ...
    addComment: async (dishId, comment) => {
      ...
      if (
        ...
        throw new Error("评论内容和图片至少需要一个");
      }

      const newComment = {
        id: Date.now(),
        content: comment.content || "", // 允许空内容
    ...
    deleteComment: async (dishId, commentId) => {
      ...
      // 更新菜品评分
      updatedDish.rating =
        updatedDish.comments.length > 0
          ? updatedDish.comments.reduce((sum, c) => sum + c.rating, 0) /
            updatedDish.comments.length
          : 0;
    ...
    updateComment: async (dishId, commentId, updatedComment) => {
      ...
      if (
        ...
        (!updatedComment.images || updatedComment.images.length === 0)
      ) {
        throw new Error("评论内容和图片至少需要一个");
      }
      ...
      dish.comments[commentIndex] = {
        ...dish.comments[commentIndex],
        content: updatedComment.content || "", // 允许空内容
